...
def run():
    ...
    # --- Lógica de Previsão (Quando o botão é clicado) ---
    if submit_button:
        ...
        if resultado in ['Obesidade Grau I', 'Obesidade Grau II', 'Obesidade Grau III']:
            ...
            st.markdown(
                ...
                "Recomenda-se avaliação clínica detalhada e acompanhamento especializado."
            )

        elif resultado == 'Sobrepeso':
            st.warning(f"Nível de Risco Estimado: {resultado}")
            st.markdown(
                "**Recomendação:** O paciente está na **faixa de alerta (Sobrepeso)**. "
                "Este é o momento ideal para ajustes de hábitos e prevenção de progressão do risco."
            )

        elif resultado == 'Peso Insuficiente':
            st.info(f"Nível de Risco Estimado: {resultado}")
            st.markdown(
                "**Recomendação:** O paciente está abaixo do peso considerado adequado pela OMS. "
                "Sugere-se avaliação nutricional para investigar causas e orientar recuperação saudável."
            )

        else:
            st.success(f"Nível de Risco Estimado: {resultado}")
            st.markdown(
                "**Recomendação:** O paciente está dentro da faixa saudável. "
                "Acompanhamento periódico e manutenção dos hábitos atuais são recomendados."
            )
