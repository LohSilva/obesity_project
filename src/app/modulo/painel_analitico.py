...
def run():  
    ...
    fig_estilo_vida = px.box(
        ...
        category_orders={'classe_peso_oms': categorias_oms} 
    )
    ...
    st.markdown("---")
    st.header("Validação do Modelo (Resultados do Treinamento)")
    st.markdown(
        "Para garantir a eficácia da correção dos dados, apresentamos abaixo as métricas visuais "
        "do modelo preditivo final treinado. Estas imagens comprovam que o modelo aprendeu padrões "
        "consistentes e identificou as variáveis comportamentais mais relevantes."
    )

    # --- Cria colunas para as imagens ficarem lado a lado ---
    col1, col2 = st.columns(2)

    with col1:
        st.subheader("Performance (Matriz de Confusão)")
        try:
            st.image(CM_PATH, caption="Onde o modelo acerta e erra", use_container_width=True)
        except NameError:
             st.error("Variável CM_PATH não definida.")
        except FileNotFoundError:
            st.warning("Imagem da Matriz não encontrada.")

    with col2:
        st.subheader("Fatores de Decisão (SHAP)")
        try:
            st.image(SHAP_PATH, caption="Variáveis de maior impacto", use_container_width=True)
        except NameError:
             st.error("Variável SHAP_PATH não definida.")
        except FileNotFoundError:
            st.warning("Imagem do SHAP não encontrada.")
